...
function getSystemPrompt(model) {
  ...
  lines.push('- IMAGE GENERATION. Emit `[GENERATE_IMAGE prompt="..." aspect_ratio="16:9"]`. CRITICAL RULE ON THE PROMPT: pass the user\'s words VERBATIM. Do NOT add style adjectives, colors, lighting, composition, mood, camera terms, medium descriptors, or "stylized/cartoon/realistic/cinematic" qualifiers UNLESS the user explicitly used those words. Do NOT try to "enhance" the prompt. Do NOT interpret the user\'s request — if they typed "gorilla tag gorilla", the prompt is literally `prompt="gorilla tag gorilla"` and nothing else. If they typed "gtag", the prompt is `prompt="gtag"`. If they typed "a cat", the prompt is `prompt="a cat"` — do NOT turn it into "a fluffy orange tabby cat sitting on a windowsill in soft morning light". The image model does the visual interpretation, not you. The ONLY thing you may add is a proper-noun clarifier IF the user\'s phrase is ambiguous (e.g. "gorilla tag" → "Gorilla Tag (the VR game)"); and ONLY the minimum needed. The aspect_ratio param is OPTIONAL — include it when the user asks for "wide", "tall", "portrait", "landscape", "16:9", "square", "1080p", "phone wallpaper", etc. Common values: "1:1", "16:9", "9:16", "4:3", "3:4", "21:9", "3:2", "2:3". Default is square if you omit it. NEVER emit GENERATE_IMAGE if the user has attached an image to this message — use EDIT_IMAGE instead.');
