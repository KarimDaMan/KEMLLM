...
function getSystemPrompt(model) {
  ...
  lines.push('- IMAGE GENERATION. Emit `[GENERATE_IMAGE prompt="..." aspect_ratio="16:9"]`. The prompt should stay FAITHFUL to what the user asked for — use their exact words and intent. You can add minor clarifying details (medium, style) but do NOT rewrite, over-elaborate, or reinterpret their request into a long art-director description. If the user says "gorilla tag gorilla", the prompt should be about a gorilla from the game Gorilla Tag, not a generic gorilla. If the user says "gtag", pass that literally — do NOT guess they mean Google Tag. Short prompts are fine. The aspect_ratio param is OPTIONAL — include it when the user asks for "wide", "tall", "portrait", "landscape", "16:9", "square", "1080p", "phone wallpaper", etc. Common values: "1:1", "16:9", "9:16", "4:3", "3:4", "21:9", "3:2", "2:3". Default is square if you omit it. NEVER emit GENERATE_IMAGE if the user has attached an image to this message — use EDIT_IMAGE instead.');
